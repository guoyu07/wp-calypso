...
import { getPost, getPostByFeedAndId } from 'state/reader/posts/selectors';
...
function setPost( post ) {
	console.error( post.ID, post.feed_ID, post );
	if ( ! post || ! post.feed_ID || ! post.ID ) {
		return false;
	}

	const cachedPost = getPostByFeedAndId( reduxGetState(), post.feed_ID, post.ID );
...
function receivePending( action ) {
	const post = { ID: action.postId, feed_ID: action.feedId };
	const currentPost = getPostByFeedAndId( reduxGetState(), post.feed_ID, post.ID );
...
function receivePostFromPage( newPost ) {
	...
	if ( newPost.feed_ID && newPost.ID && ! getPost( reduxGetState(), newPost.ID ) ) {
